...
async def get_questions(conn):
    query = await conn.execute(question.select())
...
async def vote(conn, question_id, choice_id):
    result = await conn.execute(
        choice.update().returning(
            *choice.c
        ).where(
            choice.c.question_id == question_id
        ).where(
            choice.c.id == choice_id
        ).values(
            votes=choice.c.votes+1
        )
    )
    record = await result.fetchone()
    if not record:
        msg = f"Question with id: {question_id} or choice with id: {choice_id} does not exists"
        raise RecordNotFound(msg)
